...
[Lispbox](http://common-lisp.net/project/lispbox/) を使えばいいと思います。でも、正直・・・

1.  Emacsを入れろ！(どーん)

2.  処理系は迷う必要はない。とりあえず [sbcl](http://www.sbcl.org/) を入れろ！バイナリを落とし
...
Vimな方面については、[Limp](http://www.vim.org/scripts/script.php?script_id=2219) をどうぞ、ですが、あまりおすすめしません。
...
Practical Common Lisp ([実践CommonLisp](http://www.amazon.co.jp/実践Common-Lisp-Peter-Seibel/dp/4274067211)) を読んで、ソースコードを打ち込ん
...
次に必要なのが[Hyperspec](http://www.lispworks.com/documentation/HyperSpec/Front/Contents.htm) です。ここが自分にとって第一の関門でした。これ
...
はいhyperspecの復習。第二章の[Syntax](http://www.lispworks.com/documentation/HyperSpec/Body/02_.htm) を読んでください。
...
概要については、asdf Manualの[第四章](http://common-lisp.net/project/asdf/asdf/Using-ASDF.html#Using-ASDF) からよむと良いでしょう。まあ今のと
...
-   **[Cliki.net](http://www.cliki.net):** Common Lispに関連するWikiです。…が、雑然としていて、あ
    まりメンテナンスされていません。自分は頑張って更新したり
    しているつもりです。まずはじめはCurrent Recommended
    Libraries を見るべきでしょう。

-   **[quicklisp news](http://blog.quicklisp.org/):** quicklisp の更新情報を載せているブログです。
...
-   **[quicklisp-project リポジトリ](https://github.com/quicklisp/quicklisp-projects):** quicklispへの申請を待っているプロジェ
    クトが issues にて管理人のxachにお願いを立てています。登録をする
    には、プロジェクトの重要性・新規性、マルチプラットフォーム性など
    をアピールしないといけないので、それぞれのプロジェクトの主張を見
    れてためになります。

-   **[quickdocs](http://quickdocs.org/):** 深町さんによるドキュメンテーション・API検索エンジン。ド
...
Clikiの[Currently Recommended Library](http://www.cliki.net/Current recommended libraries)は **最重要チェック項目** です。特に、
...
-   **[cl-project](https://github.com/fukamachi/cl-project):** プロジェクトのテンプレートを作るライブラリ
...
つも持っています。[日本語版WikipediaのCLOSのページ](http://ja.wikipedia.org/wiki/CLOS) は僕がかなり書きました
ので、そこの例をみると理解の助けになるでしょう。もちろん、hyperspecも
読んでください。

[MOP](http://alu.org/mop/index.html) はCltl2で提唱された考えではあるみたいですが、処理系依存の部分が残っ
...
[CFFI](http://common-lisp.net/project/cffi/) は、Cやその他の言語で書かれたライブラリを扱うためのフレームワーク
